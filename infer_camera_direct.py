...
def main():
    ...
    # 初始化飞控通信类 (后续需根据实际串口修改, 这里临时选用 /dev/ttyUSB0代替)
    uav_controller = UAVController(port='/dev/ttyUSB0', baud=115200)

    # 控制总开关: 只有当人在地面发送允许指令才会开启 AI 追踪
    AI_CONTROL_ENABLE = False
    ...
    while cap.isOpened():
        ...
        # 🚀【核心优化点】只有在进入发送队列的前一秒，才将画好完美框的图 resize 为轻量化分辨率
